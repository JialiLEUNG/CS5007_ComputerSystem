...
void *runner(void *param) {
    // decode the thread number
    unsigned int thread_no = (char*)param - (char*)0;
    // your code here
    while (running == true){
        sleep(1);
        // The process id
        ...
        // If tid is the selected thread,
        // it also prints the pid and its thread
        // number to stdout during each interval.
        if (tid == selected_thread){
            printf("pid: %lu [thread %lu] running\n", (unsigned long)pid, (unsigned long)thread_no);
        }
 ...
 * @return count number of threads actually created
 */
unsigned int createThreads(unsigned int nthreads, pthread_t threads[]) {
	...
	for (int i = 0; i < nthreads; i++){
	    ...
	    // create thread.
	    // &(threads[i]) is an alternative of threads+i.
	    if (pthread_create(threads+i, &attr, runner, NULL+i) != 0){
	        // Stops creating if pthread_create returns non-zero result.
            fprintf(stderr, "thread[%d] not created.", i);
            return count;
	    }
        count++;
	}
...
void manageThreads(unsigned int timeslice, unsigned int nthreads, const pthread_t threads[]) {
    ...
    while (true){
        for (int i = 0; i < nthreads; i++){
//            void* param = (char*)0 + i; // encode thread_no as pointer.
            // pointer encoding.
